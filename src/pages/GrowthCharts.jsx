...
import { getGrowthEntries, deleteGrowthEntry } from "../services/growthService";
...
function GrowthCharts() {
  ...
  async function handleDeleteEntry(entryId) {
    try {
      await deleteGrowthEntry(childId, entryId);
      alert("Entry deleted.");
      loadData();
    } catch (err) {
      console.error(err);
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-10 flex flex-wrap items-center justify-between gap-4">
            ...
            <button
              ...
              className="
                px-6 py-3 rounded-2xl bg-secondary text-white font-semibold
                shadow-soft hover:scale-105 transition duration-300
              "
            >
              Official IAP Chart (0-18)
            </button>
          </div>
          ...
          {showOfficial && (
            ...
            >
              <div
                className="bg-white rounded-[28px] max-w-5xl w-full max-h-[95vh] overflow-auto p-6"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-2xl font-bold text-secondary">
                    Paediatrician-friendly IAP Growth Chart — {gender === "male" ? "Boys" : "Girls"} 0-18 Years
                  </h2>
                  <button
                    onClick={() => setShowOfficial(false)}
                ...
                <img
                  src={gender === "male" ? "/charts/boys-0-18.jpg" : "/charts/girls-0-18.jpg"}
                  alt={`IAP ${gender === "male" ? "boys" : "girls"} height and weight chart 0-18 years`}
                  className="w-full h-auto rounded-xl"
                />
                <p className="text-sm text-gray-500 mt-4">
                  Source: Paediatrician-friendly IAP Growth Charts for 0-18 years (Parekh &amp; Khadilkar),
                  Indian Academy of Pediatrics — WHO standards 0-5 y, IAP 2015 charts 5-18 y.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="grid lg:grid-cols-[350px_1fr] gap-10">
            ...
            <div className="space-y-8">
              <GrowthTable records={records} onDeleteEntry={handleDeleteEntry} />
